...
abstract class Mage_Core_Controller_Varien_Action
{
    ...
    public function preDispatch()
    {
        ...
        if (!$this->getFlag('', self::FLAG_NO_START_SESSION)) {
			...
			// Flags the session as brand new
			$_isNewSession = $session->getIsNewSession();
			if (!isset($_isNewSession)) {
    ...
    {
		// Allows the form key to be ignored only ONE time and only on the Frontend Session
		$session =  Mage::getSingleton('core/session');
		if ($session->getSessionName() === 'frontend'
